...
import { tvPackageSchema, type TVPackageFormData, generateSlug } from "@/lib/admin-schemas"
import { useTVPackageItem, useTVPackagesAdmin } from "@/hooks/use-packages"
...
export default function TVPackageEditPage() {
  const { id } = useParams()
  const isNew = id === "nowy"
  const navigate = useNavigate()
  const { pkg, loading: loadingItem } = useTVPackageItem(isNew ? undefined : id)
  const { create, update } = useTVPackagesAdmin()
  ...
  } = useForm<TVPackageFormData>({
    resolver: zodResolver(tvPackageSchema),
    defaultValues: {
      type: "iptv",
      features: [],
  ...
  useEffect(() => {
    if (pkg && !isNew) {
      reset({
        slug: pkg.slug,
        name: pkg.name,
        type: pkg.type as TVPackageFormData["type"],
        tagline: pkg.tagline,
        channels: pkg.channels,
        features: pkg.features as string[],
        pricing: pkg.pricing as TVPackageFormData["pricing"],
  ...
  async function onSubmit(data: TVPackageFormData) {
    setError(null)
    const payload = {
      slug: data.slug,
      name: data.name,
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h2 className="text-2xl font-bold mb-6">
        {isNew ? "Nowy pakiet TV" : "Edytuj pakiet TV"}
      </h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <Input {...register("name")} onBlur={handleNameBlur} placeholder="np. TV Rodzinny" />
            {errors.name && <p className="text-sm text-destructive mt-1">{errors.name.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium mb-1.5">Slug *</label>
            <Input {...register("slug")} placeholder="np. tv-iptv-standard" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Liczba kanałów *</label>
            <Input type="number" {...register("channels", { valueAsNumber: true })} min={1} />
            {errors.channels && <p className="text-sm text-destructive mt-1">{errors.channels.message}</p>}
          </div>
        </div>
